...
// -------------------------------------------------------------------------
// [경희대학교 전자공학과 2022학년도 교육과정 데이터]
// 출처: 전자공학부 홈페이지(ee.khu.ac.kr) > 대학 > 교과과정 >
//       "2022년 전자정보대학 교육과정.pdf" (등록 2025-04-24, 게시글 "2022학년도 전자공학과 교육과정")
//   · 과목/학수번호/학점/이수학년/개설학기 : [별표1] 교육과정 편성표 (52과목, p.120-121)
//   · 학년별 주개설학기(동시개설 과목 슬롯 판정) : [별표3] 학년별 교과목 편성표 (p.123)
//   · 선수과목(prerequisites)             : [별표4] 선수과목 지정표 (41건, p.124-125)
//   · 트랙(tracks)                        : [별표2] 전자공학 이수체계도 (반도체및파동/회로및시스템/통신및신호처리 3대분류, p.122) + 과목 성격 기반 5트랙 재분류
//   · 졸업이수학점 130 (전공기초30+전공필수26+전공선택28=84, 단일전공 기준) : [표1] 이수학점 편성표(p.115), 시행세칙 제3조
// 비고: 개설학기가 1·2학기 동시개설인 과목은 [별표3] 학년별 편성표의 주(主)개설학기 기준으로 1개 슬롯 배치.
// 특이사항: 별표1의 개설학기(O표시) 판독 해상도 한계로 일부 전공선택 과목(EE341~EE370 구간)의 정확한 주개설학기는
//           별표3 대조 및 문서 내 배치 순서를 근거로 추정하였음. 자세한 내용은 tasks/STATUS.md 참고.
// -------------------------------------------------------------------------
window.CURRICULA["EE"]["2022"] = {
    gradReq: 130, // 최저 졸업이수학점 (전자공학 교육과정 시행세칙 제3조, [표1])
    courses: [
        // ===== 전공기초 (30학점) =====
        // 1학년 1학기
        ...
        {code:"SWCON104", name:"웹/파이썬프로그래밍", type:"전공 기초", credits:3, year:1, sem:1, track:"COMP"},
        // 1학년 2학기
        ...
        // ===== 전공필수 (26학점) =====
        // 2학년 1학기
        {code:"EE212", name:"AdventureDesign", type:"전공 필수", credits:3, year:2, sem:1, track:"CSY"},
        ...
        // 3학년 2학기
        {code:"EE341", name:"디지털통신", type:"전공 선택", credits:3, year:3, sem:2, track:"CS"},
        {code:"EE441", name:"정보및부호이론", type:"전공 선택", credits:3, year:3, sem:2, track:"CS", isMinor:true},
        {code:"EE342", name:"디지털신호처리", type:"전공 선택", credits:3, year:3, sem:2, track:"CS"},
        {code:"EE361", name:"디지털회로설계및언어", type:"전공 선택", credits:3, year:3, sem:2, track:"CSY", isMinor:true},
        {code:"EE364", name:"마이크로프로세서", type:"전공 선택", credits:3, year:3, sem:2, track:"CSY"},
        {code:"EE365", name:"전자회로2", type:"전공 선택", credits:3, year:3, sem:2, track:"CSY"},
        {code:"EE363", name:"자동제어", type:"전공 선택", credits:3, year:3, sem:2, track:"CSY", isMinor:true},
        {code:"EE343", name:"DSP실험", type:"전공 선택", credits:2, year:3, sem:2, track:"CS", isMinor:true},
        {code:"EE367", name:"임베디드시스템설계", type:"전공 선택", credits:3, year:3, sem:2, track:"CSY", isMinor:true},
        ...
        {code:"EE463", name:"VLSI설계", type:"전공 선택", credits:3, year:4, sem:1, track:"CSY", isMinor:true},
        ...
        {code:"EE484", name:"실전문제연구종합설계", type:"전공 선택", credits:3, year:4, sem:1, track:"CSY", isMinor:true},
        // 4학년 2학기
    ...
    prerequisites: [
        ...
        {target:"EE213", prereqs:["SWCON104"]},             // 객체지향프로그래밍및실습 ← 웹/파이썬프로그래밍
        ...
        {target:"EE207", prereqs:["AMTH1003"]},             // 기초회로실험 ← 미분적분학2
        {target:"EE210", prereqs:["EE209"]},                // 신호와시스템 ← 논리회로
        {target:"CSE302", prereqs:["EE203"]},               // 컴퓨터네트워크 ← 물리전자
        ...
        {target:"EE370", prereqs:["SWCON104"]},             // 소프트웨어랩 ← 웹/파이썬프로그래밍
        ...
        {target:"EE442", prereqs:["EE211"]},                // 이동통신 ← 확률및랜덤변수
        ...
        {target:"EE497", prereqs:["EE212", "EE207", "EE210", "EE213"]} // 종합설계 ← Adventure Design, 기초회로실험, 신호와시스템, 객체지향프로그래밍및실습 (모두 수강)
    ],

    // [별표2] 전자공학 이수체계도(반도체및파동/회로및시스템/통신및신호처리) 기반 진로 트랙별 추천 과목
    tracks: {
        // 반도체/공정 (반도체 및 파동)
        ...
        // 인공지능/신호처리
        ai: ["AMTH1002", "AMTH1004", "SWCON104", "EE213", "EE210", "EE371", "EE444"],
        // 통신/신호처리
